 ...
 */
export function mkdir(path: string, mode?: number | string): void
/**
 ...
 */
export function isDirectory(path: string, link: Boolean): Boolean
/**
 * 目标是否文件
 * @param path 目标路径
 * @param link 是否检测符号链接(默认false) 为true时,文件若是符号链接则返回false
 */
export function isFile(path: string, link: Boolean): Boolean
/**
 * 目标是否符号连接
 */
...
export * as promises from './filePromises'
